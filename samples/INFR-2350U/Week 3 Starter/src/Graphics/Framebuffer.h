...
public:
	...
	//Renders the framebudder to our FullScreenQuad
	void RenderToFSQ() const;

	//Draw the contents of the framebuffer to the back buffer
	void DrawToBackbuffer();

	//Clear the framebuffer using our clear flag
	void Clear();
	//Checks to make sure the framebuffer is quad
	bool CheckFBO();

	//Initializes fullscreen quad
	//*CreatesVAO for full screen quad
	//*covers -1 to 1 range
...
protected:
	...
	GLenum _warp = GL_CLAMP_TO_EDGE;

	//Clearflag is nothing by defualt
	GLbitfield _clearFlag = 0;

	//Is the framebuffer initlized
	bool _isInit = false;
	//Depth attachment?
