...
{
	//Unloads the depth targets
	Unload();
}

void DepthTarget::Unload()
{
	//Delates the texture ar the specific handle
...
void Framebuffer::Init()
{
	...
	if (_depthActive)
	{
		//becuase we have depth we need to clear our depth
		...
		glTextureParameteri(_depth._texture.GetHandle(), GL_TEXTURE_WRAP_S, _warp);
		glTextureParameteri(_depth._texture.GetHandle(), GL_TEXTURE_WRAP_T, _warp);
...
{
	//Resizeing the textures to number of attachments
	_color._textures.resize(_color._numAttachments + 1);
	//Add the formant
	_color._formats.push_back(format);
	//Add teh color attachment buffer number
...
void Framebuffer::DrawToBackbuffer()
{
	...
	//Blits the frambuffer to the back buffer
