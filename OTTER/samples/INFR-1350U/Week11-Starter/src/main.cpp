...
bool InitGLFW() {
	...
	window = glfwCreateWindow(800, 800, "INFR2350U", nullptr, nullptr);
...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		int lightAmbient = 0;
		int lightSpecular = 0;
		int lightAmbientSpecular = 0;
		int lightAmbientSpecularToon = 0;
		...
		shader->SetUniform("u_NoLighting", lightNo);
		shader->SetUniform("u_Ambient", lightAmbient);
		shader->SetUniform("u_Specular", lightSpecular);
		shader->SetUniform("u_AmbientSpecular", lightAmbientSpecular);
		shader->SetUniform("u_AmbientSpecularToonShading", lightAmbientSpecularToon);

		bool lightsOff = false;
		bool AmbientOff = false;
		...
		imGuiCallbacks.push_back([&]() {
			...
			{
				shader->SetUniform("u_NoLighting", lightNo = 2);
				shader->SetUniform("u_Ambient", lightAmbient = 0);
				shader->SetUniform("u_Specular", lightSpecular = 0);
				shader->SetUniform("u_AmbientSpecular", lightAmbientSpecular = 0);
				shader->SetUniform("u_AmbientSpecularToonShading", lightAmbientSpecularToon = 0);

			}
			// Ambient lighting only | An button to only turn on the ambient light 
			if (ImGui::Button("Ambient lights only"))
			{
				shader->SetUniform("u_NoLighting", lightNo = 0);
				shader->SetUniform("u_Ambient", lightAmbient = 2);
				shader->SetUniform("u_Specular", lightSpecular = 0);
				shader->SetUniform("u_AmbientSpecular", lightAmbientSpecular = 0);
				shader->SetUniform("u_AmbientSpecularToonShading", lightAmbientSpecularToon = 0);
			}
	
			// Specular lighting only | An button to only turn on the Specular light  
			if (ImGui::Button("Specular lights only"))
			{
				shader->SetUniform("u_NoLighting", lightNo = 0);
				shader->SetUniform("u_Ambient", lightAmbient = 0);
				shader->SetUniform("u_Specular", lightSpecular = 2);
				shader->SetUniform("u_AmbientSpecular", lightAmbientSpecular = 0);
				shader->SetUniform("u_AmbientSpecularToonShading", lightAmbientSpecularToon = 0);
			}
			...
			if (ImGui::Button("Ambient & Specular"))
			{
				...
				shader->SetUniform("u_Ambient", lightAmbient = 0);
				shader->SetUniform("u_Specular", lightSpecular = 0);
				shader->SetUniform("u_AmbientSpecular", lightAmbientSpecular = 2);
				shader->SetUniform("u_AmbientSpecularToonShading", lightAmbientSpecularToon = 0);
			}
			...
			if (ImGui::Button("Ambient & Specular & Toon Shading"))
			{
				...
				shader->SetUniform("u_Specular", lightSpecular = 0);
				shader->SetUniform("u_AmbientSpecular", lightAmbientSpecular = 0);
				shader->SetUniform("u_AmbientSpecularToonShading", lightAmbientSpecularToon = 2);
			}

			// Toon Shader 2:08:31 https://drive.google.com/file/d/15jnWKCUPfJDSvlpoTfLj8jxK6-crZhTF/preview
		...
		Texture2D::sptr diffuseGrass = Texture2D::LoadFromFile("images/Grass.png");
		Texture2D::sptr diffuseTable = Texture2D::LoadFromFile("images/Table.png");
		Texture2D::sptr diffuseFlower = Texture2D::LoadFromFile("images/Flower.png");
		Texture2D::sptr diffuseBench = Texture2D::LoadFromFile("images/Bench.png");
		Texture2D::sptr diffuseTree = Texture2D::LoadFromFile("images/Tree.png");
		...
		// Create a material and set some properties for it
		ShaderMaterial::sptr materialGrass = ShaderMaterial::Create();
		materialGrass->Shader = shader;
		materialGrass->Set("s_Diffuse", diffuseGrass);
		materialGrass->Set("s_Diffuse2", diffuse2);
		materialGrass->Set("s_Specular", specular);
		materialGrass->Set("u_Shininess", 8.0f);
		materialGrass->Set("u_TextureMix", 0.0f);
		...
		materialTable->Set("s_Diffuse2", diffuse);
		materialTable->Set("s_Specular", specular);
		materialTable->Set("u_Shininess", 8.0f);
		materialTable->Set("u_TextureMix", 0.2f);
		...
		materialBench->Set("s_Diffuse2", diffuse);
		materialBench->Set("s_Specular", specular);
		materialBench->Set("u_Shininess", 8.0f);
		materialBench->Set("u_TextureMix", 0.2f);

		//Flower
		ShaderMaterial::sptr materialFlower = ShaderMaterial::Create();
		materialFlower->Shader = shader;
		materialFlower->Set("s_Diffuse", diffuseFlower);
		materialFlower->Set("s_Diffuse2", diffuseGrass);
		materialFlower->Set("s_Specular", specular);
		materialFlower->Set("u_Shininess", 8.0f);
		materialFlower->Set("u_TextureMix", 0.2f);

		//Tree
		ShaderMaterial::sptr materialTree = ShaderMaterial::Create();
		materialTree->Shader = shader;
		materialTree->Set("s_Diffuse", diffuseTree);
		materialTree->Set("s_Diffuse2", diffuse);
		materialTree->Set("s_Specular", specular);
		materialTree->Set("u_Shininess", 8.0f);
		materialTree->Set("u_TextureMix", 0.2f);
		
		//Objects 
		//Ground
		GameObject sceneObj = scene->CreateEntity("scene_geo"); 
		{
			VertexArrayObject::sptr sceneVao = ObjLoader::LoadFromFile("models/Ground.obj");
			sceneObj.emplace<RendererComponent>().SetMesh(sceneVao).SetMaterial(materialGrass);
			sceneObj.get<Transform>().SetLocalPosition(0.0f, 0.0f, -4.0f);
			sceneObj.get<Transform>().SetLocalRotation(90.0f, 0.0f, 0.0f);
			sceneObj.get<Transform>().SetLocalScale(0.5f, 0.5f, 0.5f);
		}

		//Table
		GameObject obj2 = scene->CreateEntity("Table");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/Table.obj");
			obj2.emplace<RendererComponent>().SetMesh(vao).SetMaterial(materialTable);
		...
		GameObject obj3 = scene->CreateEntity("Bench");
		{
			...
			obj3.emplace<RendererComponent>().SetMesh(vao).SetMaterial(materialBench);
			obj3.get<Transform>().SetLocalPosition(0.0f, -6.0f, -1.0f);
			obj3.get<Transform>().SetLocalRotation(90.0f, 0.0f, 0.0f);
			obj3.get<Transform>().SetLocalScale(0.7f, 0.7f, 0.7f);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(obj3);
		}


		GameObject obj5 = scene->CreateEntity("Flower");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/Flower.obj");
			obj5.emplace<RendererComponent>().SetMesh(vao).SetMaterial(materialFlower);
			obj5.get<Transform>().SetLocalPosition(-5.2f, 0.0f, -1.5f);
			obj5.get<Transform>().SetLocalRotation(90.0f, 0.0f, 90.0f);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(obj5);
		}

		GameObject obj4 = scene->CreateEntity("Balloon");
		{
			...
			obj4.get<Transform>().SetLocalPosition(-4.2f, -4.2f, 1.0f);
			...
			pathing->Points.push_back({ -4.2f, -4.2f, 0.0f });
			pathing->Points.push_back({ 4.2f, -4.2f, 0.0f });
			pathing->Points.push_back({ 4.2f,  4.2f, 0.0f });
			pathing->Points.push_back({ -4.2f,  4.2f, 0.0f });
			pathing->Speed = 3.0f;
		...
		GameObject obj6 = scene->CreateEntity("Tree");
		{
			...
			obj6.emplace<RendererComponent>().SetMesh(vao).SetMaterial(materialTree);
			obj6.get<Transform>().SetLocalPosition(0.0f, 8.0f, 5.0f);
			obj6.get<Transform>().SetLocalRotation(90.0f, 0.0f, 110.0f);
			obj6.get<Transform>().SetLocalScale(0.6f, 0.6f, 0.6f);
		}
		
		GameObject obj7 = scene->CreateEntity("Tree2");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/Tree.obj");
			obj7.emplace<RendererComponent>().SetMesh(vao).SetMaterial(materialTree);
			obj7.get<Transform>().SetLocalPosition(4.0f, -8.0f, 5.0f);
			obj7.get<Transform>().SetLocalRotation(90.0f, 0.0f, 20.0f);
			obj7.get<Transform>().SetLocalScale(0.6f, 0.6f, 0.6f);
		}
